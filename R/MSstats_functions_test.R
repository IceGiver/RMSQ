...
maxqFilterAndJoin = function(data, keys, remove_proteingroups=REMOVE_PROTEINGROUPS){
  ## get rid of CONTAMINANTS
  if(REMOVE_PROTEINGROUPS){
    data_selected = data[grep("CON__|REV__|;",data$Proteins, invert=T),]
  }else{
    data_selected = data[grep("CON__|REV__",data$Proteins, invert=T),]
  }
  
  unique_data = unique(data$Raw.file)
  unique_keys = unique(keys$RawFile)
  keys_not_found = paste(setdiff(unique_keys, unique_data),collapse='\n')
  data_not_found = paste(setdiff(unique_data, unique_keys),collapse='\n')
  cat(sprintf("keys not in data file:\n%s", keys_not_found))
  cat(sprintf("data not in keys file:\n%s", data_not_found))
...
maxqToWide = function(data){
  ...
  data_selected_wide = dcast(data=data, formula=Proteins + Sequence + Charge ~ Run+IsotopeLabelType, value.var="Intensity",fun.aggregate=median)
  data_selected_wide
}

normalizeSingle = function(d, NORMALIZATION_METHOD="scale", FILL_MISSING=F, ref_name=''){
  unique_key_cols = unique(d[,c("Raw.file","Condition","BioReplicate","Run","NormalizationGroup")])
  unique_val_cols = c("Proteins","Sequence","Charge")
  
  print(nrow(unique(d[,c("Proteins","Sequence","Charge")])))
  
  d_w = maxqToWide(d)
  d_w_n = data.frame(d_w[,1:3],normalizeBetweenArrays(as.matrix(d_w[,4:ncol(d_w)]), method=NORMALIZATION_METHOD))  
  colnames(d_w_n) = colnames(d_w)
  
  if(FILL_MISSING){
    print("FILLING WITH MINIMUM VALUES")
    d_w_n = fillMissing(dw=d_w_n,rng_data=4:ncol(d_w_n))
  }
  d_w_n_m = melt(d_w_n, id.vars=c("Proteins","Sequence","Charge"), value.name="Intensity",variable.name='Run', na.rm=T)
...
BACKGROUND_REF = "P99024|Q7TMM9|P68372|P99024|P68369|P68372|P99024|P68372|P99024|Q7TMM9|Q7TMM9|P68369|P60710" ## TUBULINS & GAPDH MOUSE
...
BASE_NAME=sprintf("BRG1_TEST_MOCK_PERD_N-%s_M-%s",NORMALIZE_BATCH, FILL_MISSING)

contrast_file = 'data/input/031114-SH-22-69/BRG1_ALL_TOD0.txt'
BASE_NAME=sprintf("BRG1_TEST_ALLTOD0_N-%s_M-%s",NORMALIZE_BATCH, FILL_MISSING)

contrast_file = 'data/input/031114-SH-22-69/BRG1_MOCK_contrasts.txt'
BASE_NAME=sprintf("BRG1_TEST_ALLTOMOCK_N-%s_M-%s",NORMALIZE_BATCH, FILL_MISSING)
...
main = function(){
  ...
  if(PREPROCESS){
    ...
    if(NORMALIZE){
      ...
      if(NORMALIZE_BATCH=="CONDITION"){
        ...
        data_n = normalizeSingle(data_ff, NORMALIZATION_METHOD=NORMALIZATION_METHOD, FILL_MISSING)
        d_bait = data_n[data_n$NormalizationGroup=='BAIT',]
        d_bait = normalizeToReference(d_bait, keys, ref_protein=BAIT_REF, PDF=T, PRINT_DIR=PRINT_DIR)
        data_n = rbind(data_n[data_n$NormalizationGroup!='BAIT',],d_bait)
      }
  ...
  pos_proteins = mss_out[(!is.na(mss_out$log2FC) &  mss_out$log2FC >= 0) | mss_out$Protein==BAIT_REF, 'Protein']
  mss_F = mss_out[mss_out$Protein %in% pos_proteins, ]
  heat_data_w = plotHeat(mss_F, gsub('.txt','-res.pdf',out_file))
 
  sign_proteins = mss_out[(!is.na(mss_out$log2FC) & mss_out$adj_pvalue <= FDR & mss_out$log2FC >= LFC) | mss_out$Protein==BAIT_REF, 'Protein']
