...
castMaxQToWide = function(d_long, aggregateFun=sum){
  data_w = dcast.data.table( Proteins + Sequence + Charge ~ Raw.file + IsotopeLabelType, data=d_long, value.var='Intensity', fun.aggregate=aggregateFun, fill = NA)
  #setnames(data_w,2,'Sequence')
  return(data_w)
}

castMaxQToWidePTM = function(d_long, aggregateFun=sum){
  data_w = dcast.data.table( Proteins + Modified.sequence + Charge ~ Raw.file + IsotopeLabelType, data=d_long, value.var='Intensity', fun.aggregate=aggregateFun, fill=NA)
  setnames(data_w,2,'Sequence')
  return(data_w)
}
...
#getIsotopeLabel('VE20130426_04_dbdbk_Light')

getFileNameWithoutLabel = function(str){
  last_idx = regexpr("\\_[^\\_]*$", str)[1]
  return(substr(str,0,last_idx-1))
}
#getFileNameWithoutLabel('VE20130426_04_dbdbk_Light')
...
meltMaxQToLong = function(data_w){
  ...
  ## get last index of underscore generated by the wide cast
  
  data_l$IsotopeLabelType=apply(data_l,1,function(x) getIsotopeLabel(x['Raw.file']))
  data_l$Raw.file=apply(data_l,1,function(x) getFileNameWithoutLabel(x['Raw.file']))
  return(data_l)
}
