...
#                       FeatureSelection=FALSE,missing.action ="none", summaryMethod="linear",equalFeatureVar=TRUE){
#         
#         added: missing.action ="none", summaryMethod="linear", equalFeatureVar=TRUE,
#         removed: lambda=seq(0,1.4,0.1),eta=0.05,
#         
#groupComparison<-function(contrast.matrix=contrast.matrix,data=data)
#                 
#                 removed entirely: labeled=TRUE, scopeOfBioReplication="restricted", 
#         scopeOfTechReplication="expanded", interference=TRUE,
#         
#         moved to dataProcess: missing.action = "nointeraction", equalFeatureVar=TRUE,

runMSstats = function(dmss, contrasts, config){
   versionchar <- unlist(strsplit(config$msstats$version, 
                               "/"))[length(unlist(strsplit(config$msstats$version, "/")))-1]
#        cat(sprintf('>>checking MSSTATS %s VERSION\n', versionchar))
# MSstats.daily_2_3_5
# MSstats.daily_3_0_1
...
# for version 2.4: 
#          if(versionchar == "MSstats.daily_2_3_7"){
                  if(config$msstats$versionKey == 2 ){
                  mssquant = dataProcess(dmss, normalization=F, fillIncompleteRows = T)         
          }

# for version 3.0:
# need to add summaryMethod to config as $data?
#        if(versionchar == "MSstats.daily_3_0_3"){
         if(config$msstats$versionKey == 3 ){
         mssquant = dataProcess(dmss, normalization=F, fillIncompleteRows = T, missing.action ="none", 
                                summaryMethod = config$msstats$summaryMethod, equalFeatureVar=TRUE) #$ProcessedData  
        }
 ...
 #   cat('\t Got here \t%s\n')
 # quit()  
 path_to_object_inputs <- ""
 save_object <- function(object_name){
         save(list = c(object_name), 
              file = paste0(path_to_object_inputs, object_name) , ascii = F)         
 }
 #cat('\t Got here \t%s\n')
#    if(versionchar == "MSstats.daily_2_3_5"){
        if(config$msstats$versionKey == 2 ){
         #   cat('\t Got here \t%s\n')
 ...
 #   mssquant235 <- mssquant
  #  cat('\t Got here \t%s\n')
 #  save_object("mssquant235")
#    quit()
    }
#    if(versionchar == "MSstats.daily_3_0_1"){
    if(config$msstats$versionKey == 3 ){
    mssquant = dataProcess(dmss, normalization=config$msstats$normalization_method, nameStandards=normalization_refs,
                                  fillIncompleteRows=T,missing.action ="none", 
                           summaryMethod = config$msstats$summaryMethod, equalFeatureVar=TRUE) #$ProcessedData
#     mssquant30 <- mssquant
#           save_object("mssquant30")
#  quit()
    }
    }else{
...
#        if(versionchar == "MSstats.daily_2_3_5"){ 
                if(config$msstats$versionKey == 2 ){
                mssquant = dataProcess(dmss, normalization=config$msstats$normalization_method , 
                                       fillIncompleteRows = F, betweenRunInterferenceScore = F, FeatureSelection = F)
        }
#        if(versionchar == "MSstats.daily_3_0_1"){ 
        if(config$msstats$versionKey == 3 ){
            mssquant = dataProcess(dmss, normalization=config$msstats$normalization_method , 
                        fillIncompleteRows = F, betweenRunInterferenceScore = F, FeatureSelection = F, 
                        missing.action ="none", summaryMethod = config$msstats$summaryMethod, equalFeatureVar=TRUE) #$ProcessedData
        }
    } 
...
write.table(mssquant$ProcessedData, file=gsub('-results.txt','-mss-normalized.txt',config$files$output), eol="\n", sep="\t", quote=F, row.names=F, col.names=T)
#                if(versionchar == "MSstats.daily_2_3_5"){ 
        if(config$msstats$versionKey == 2 ){
...
#                if(versionchar == "MSstats.daily_3_0_1"){ 
                        if(config$msstats$versionKey == 3 ){
results = groupComparison(data = mssquant, contrast.matrix = contrasts)$ComparisonResult  # check to see if this $Comparison needs to be here to pull proper return value. 
#                          labeled = as.logical(config$msstats$labeled), removed - ??
#                          scopeOfBioReplication = config$msstats$scopeOfBioReplication, hard wired to ‘expanded'
#                          scopeOfTechReplication = config$msstats$scopeOfTechReplication, removed - ??
#                          interference = as.logical(config$msstats$interference), removed - ??
#                          equalFeatureVar = as.logical(config$msstats$equalFeatureVar), moved to data Process.
#                          missing.action = config$msstats$missing_action)$ComparisonResult moved to data Process.
                }
