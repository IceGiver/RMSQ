...
dataProcess<-function(raw,logTrans=2, normalization="equalizeMedians",nameStandards=NULL, betweenRunInterferenceScore=FALSE,address="", fillIncompleteRows=TRUE, FeatureSelection=FALSE,missing.action ="none", summaryMethod="linear",equalFeatureVar=TRUE){
  ...
  write.table(processout, file=finalfile,row.names=FALSE)
	...
	if(missing.action=="MBimpute"){
		...
		## endogenous only
		tempwork<-work[work$LABEL=="L",]
		tempwork<-tempwork[!is.na(tempwork$ABUNDANCE),]
		
		tempwork$FEATURE<-factor(tempwork$FEATURE)

		datamat = dcast( PROTEIN + FEATURE ~ RUN, data=tempwork, value.var='ABUNDANCE', keep=TRUE) 
		
		dataimputed<-.MBimpute(datamat, my.pi=0.05, compute_pi=TRUE)

		data_l = melt(dataimputed$y_imputed, id.vars=c('PROTEIN',"FEATURE"))
		...
		data_l<-merge(data_l, feature, by="FEATURE")
		data_l$LABEL<-"L"
	...
	rqdata<-try(.runQuantification(work,summaryMethod,equalFeatureVar),silent=TRUE)
	...
	if(class(rqdata)=="try-error") {
	  ...
	  rqall<-NULL
		...
		if(summaryMethod=="skyline"){
			...
			rqall<-merge(rqdata, lab, by="SUBJECT_ORIGINAL")
  ...
  ## return work data.frame	and run quantification
	...
	processedquant<-list(ProcessedData=work, RunlevelData=rqall, SummaryMethod=summaryMethod)
...
########################################################
.runQuantification<-function(data,summaryMethod,equalFeatureVar){
...
#      sub$SUBJECT_ORIGINAL<-factor(sub$SUBJECT_ORIGINAL)
	...
	if(summaryMethod=="skyline"){
		...
		if(label){
			message("* Skyline uses log2(sum of intensities) summation. For log2(sum of intensities) summarization with label-based experiment, ratio between endogenous intensity and reference intensity is used.")
		}
		...
		for(i in 1: nlevels(data$PROTEIN)){
 			...
 			## 1.remove runs which have any missing values
 			sub$FEATURE<-factor(sub$FEATURE)
			...
			if(nrow(sub)==0){
				...
				message(paste("* All replicates in ",unique(sub$PROTEIN), " have any missing values. Can't summarize by log2 (sum of intensities)."))
...
.ttest.logsum<-function(contrast.matrix,data,origGroup){
 ...
 #       }else{
          ...
          ## t test
          sumresult<-try(t.test(datasub$ABUNDANCE~datasub$GROUP_ORIGINAL),silent=TRUE)
