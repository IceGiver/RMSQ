...
# calculate PI or used one passed in as parameter:
  ...
  y_imputed <- NULL
  name<-NULL
  runname<-colnames(m)[c(3:ncol(m))]
  
  cat("Imputing...\n")

  for (k in 1:length(all.proteins)){
  	
  	message(paste("Imputing missing values for protein ",all.proteins[k], "(",k," of ",length(all.proteins),")"))
  	
    y_raw<-m[which(m$PROTEIN %in% all.proteins[k]),c(3:ncol(m))]
    
    n.peptide <- nrow(y_raw)
  
  	## if the run has all NA, remove that run for analysis
  	countallmissing<-apply(y_raw, 2, function(x) sum(is.na(x)))
  	if(any(countallmissing==n.peptide)){
  		y_raw<-y_raw[,countallmissing!=n.peptide]
  	}
    
    ## if there is one feature, and with only one value
    if (is.null(nrow(y_raw))) next
    ...
    peptide <-rep(rep(1:n.peptide, each=ncol(y_raw)))
    ...
    go <- try(.protein_var(y_raw), silent=TRUE) # yuliya: function, see below : here used group information
    ...
    if (n.peptide != 1){
      ...
      X  <- model.matrix(~f.peptide+f.run, contrasts = list(f.peptide="contr.sum",f.run="contr.sum") )
    } else {
    
      ## !! need to check : f.treatment?
      X <- model.matrix(~f.run, contrasts=list(f.run="contr.sum"))
    }
    ...
    beta <- try(drop(solve(t(X.c) %*% X.c) %*% t(X.c) %*% y.c),silent=TRUE)
    
    if(class(beta)=="try-error") {
    	## for example, there are multiple features, however, too many missing values, so lack of measurements. need to simple model
       X <- model.matrix(~f.run, contrasts=list(f.run="contr.sum"))
    ...
    if(n.peptide==1){ ## need to check
      y.predict <- model.matrix(~f.run,contrasts = list(f.run="contr.sum"))%*% beta
    } else {
      y.predict <- try(model.matrix(~f.peptide+f.run,contrasts = list(f.peptide="contr.sum"))%*% beta, silent=TRUE)
      
      if(class(y.predict)=="try-error") {
      	y.predict <- model.matrix(~f.run,contrasts = list(f.run="contr.sum"))%*% beta
      } 
    }
 ...
 #   if (!is.null(dim(I_GRP))) I_GRP = det(I_GRP)
    ...
    name<-prot.info[prot.info$PROTEIN==all.proteins[k],]
  	...
  	if(any(countallmissing==n.peptide)){
  		...
  		final<-data.frame(matrix(NA, nrow=n.peptide, ncol=N.run))
  		colnames(final)<-runname
  		
  		for(j in 1:ncol(y.impute.return)){
  			final[,colnames(final)==colnames(y.impute.return)[j]]<-y.impute.return[,j]
  		}
  		
  		y.impute.return<-cbind(name,final)
...
#             2) number of missing and present values for each peptide
  ...
  while (PI<=0){
    ...
    PI <- fit$y[fit$x==max(pepmean)]
    count <- count + 1
    if (count > 4) break ## when smooth_span<0, can't calculate
  }
...
#  n.u.treatment <- length(unique(treatment))
  ...
  if (n.peptide != 1){
  	...
  	## however, f.treatment should be f.run
  	## however, one observation per run. singular matrix
    X  <- model.matrix(~f.peptide+f.run, contrasts = list(f.peptide="contr.sum") )
  } else {
  	## !! need to check
    X <- model.matrix(~f.run, contrasts=list(f.run="contr.sum"))
  }
